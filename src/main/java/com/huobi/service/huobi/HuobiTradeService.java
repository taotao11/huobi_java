...
public class HuobiTradeService implements TradeClient {
   ...
   * @param clientOrderId 用户自编订单号，必须8小时内已有该订单存在，否则下次下单时不允许用此值
   * @return -1	order was already closed in the long past (order state = canceled, partial-canceled, filled, partial-filled)
   ...
   * @param ids 订单编号列表（order-ids和client-order-ids必须且只能选一个填写，不超过50张订单），建议通过order-ids来撤单，比client-order-ids更快更稳定
   * @return
   */
  @Override
   ...
   * 通过API创建的订单返回order-id，按此order-id查询订单还是返回base-record-invalid是因为系统内部处理有延迟，但是不影响成交。建议您后续重试查询或者通过订阅订单推送WebSocket消息查询。
   * @param orderId 订单ID，填在path中
   * @return
   */
  @Override
